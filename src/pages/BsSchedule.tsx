...
const BsSchedule: React.FC = () => {
  ...
  const { data, refetch, isFetching, isError } =
    useGetBseVoltageInfoQuery(bsNumber);
  ...
  const createCandleData = (
    voltageData: { x: Date; y: number }[],
    ...
    currentCandleStart.setMinutes(
      Math.floor(currentCandleStart.getMinutes() / minutesInterval) *
        minutesInterval
    );
    ...
    voltageData.forEach((item) => {
      ...
      if (itemTime >= candleEndTime) {
        ...
        while (itemTime >= candleEndTime) {
          ...
          candleEndTime.setMinutes(
            candleEndTime.getMinutes() + minutesInterval
          );
        }
  ...
  useEffect(() => {
    ...
    const statusChangeAlarms = getStatusChangeAlarms(
      data.alarms_history || []
    ...
    const markersData = customMarkers.map(
      (marker: CustomMarker, index: number) => ({
        x: [marker.x],
        y: [marker.y],
        name: marker.text,
        mode: "markers+text",
      ...
      mainChartData = [
        {
          ...
          hoverinfo: "text",
          hovertext: voltageData.map(
            (item: { x: Date; y: number; status: string }) =>
    ...
    const traces = [
      ...mainChartData,
      {
        ...
        type: "scatter",
        marker: {
          color: alarmData.map((item: { type: string }) =>
            getAlarmColor(item.type, darkMode)
          ),
        ...
        hoverinfo: "text",
        hovertext: alarmData.map(
          (item: { x: Date; type: string; status: string | null }) =>
  ...
  return (
    <div
      className={`p-6 mt-16 rounded-lg max-h-[90vh] overflow-y-scroll shadow-md ${darkMode ? "bg-gray-900 text-gray-100" : "bg-white"}`}
    >
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center ml-2">
              <input
                ...
                className="mr-1"
              />
              <label htmlFor="showMA" className="mr-2">
                SMA
              </label>
              <input
                type="number"
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="flex items-center">
              <span
                ...
                style={{ background: darkMode ? "#4ADE80" : "#10B981" }}
              ></span>
      ...
      <div className="relative w-full h-[500px]" ref={plotRef}>
        {isFetching ? (
          ...
          <div
            className={`absolute inset-0 flex items-center justify-center ${darkMode ? "text-gray-400" : "text-gray-500"}`}
          >
            {isError
              ? "Ошибка загрузки данных"
              : "Данные по станции отсутствуют"}
          </div>
        ) : plotData.length > 0 ? (
          <Plot
            data={plotData}
      ...
      {data?.voltage_history && data.voltage_history.length > 0 && (
        <div
          className={`grid grid-cols-1 gap-4 mt-6 md:grid-cols-4 ${darkMode ? "text-gray-100" : ""}`}
        >
          <div
            className={`p-4 rounded-lg ${darkMode ? "bg-gray-800" : "bg-gray-50"}`}
          >
            ...
            <div className="text-2xl font-semibold">
              {data.voltage_history[
                data.voltage_history.length - 1
              ].value.toFixed(2)}{" "}
              В
            </div>
          </div>
          ...
          >
            ...
            <div className="text-2xl font-semibold">
              {Math.min(
                ...data.voltage_history.map((item) => item.value)
              ).toFixed(2)}{" "}
              В
            </div>
          </div>
          ...
          >
            ...
            <div className="text-2xl font-semibold">
              {(
                data.voltage_history.reduce(
                  (sum, item) => sum + item.value,
                  0
                ) / data.voltage_history.length
              ).toFixed(2)}{" "}
              В
            </div>
          </div>
      ...
      {customMarkers.length > 0 && (
        <div className="mt-6">
          <h3
            className={`mb-2 text-lg font-medium ${darkMode ? "text-gray-100" : ""}`}
          >
            Добавленные маркеры
          </h3>
      ...
      {data?.alarms_history && data.alarms_history.length > 0 && (
        <div className="mt-6">
          ...
          <div
            className={`overflow-auto rounded-lg border ${darkMode ? "border-gray-700" : "border-gray-200"}`}
          >
            <table
              ...
              >
                {data.alarms_history
                  ...
                  .map((alarm, idx) => (
                    ...
                    >
                      <td
                        className={`px-6 py-4 whitespace-nowrap text-sm ${darkMode ? "text-gray-100" : "text-gray-900"}`}
                      >
                        <div className="flex items-center">
                          <span
                            className="inline-block w-3 h-3 mr-2 rounded-full"
                            style={{
                              background: getAlarmColor(alarm.type!, darkMode),
                            }}
                          ></span>
                          {alarm.type}
                        </div>
